...
SOURCES=(
    ...
    tb/integration/tb_rv32i_system_trap_mret.sv
)

TOP="tb_rv32i_system_trap_mret"
...
echo
echo "[1/3] Verilator system-trap/MRET lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/system_trap_mret_verilator.log

echo
echo "[2/3] Icarus system-trap/MRET compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/system_trap_mret_iverilog.log

echo
echo "[3/3] System-trap/MRET self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/system_trap_mret.log

echo
echo "System-trap/MRET regression: PASS"
