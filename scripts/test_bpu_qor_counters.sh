...
SOURCES=(
    ...
    tb/integration/tb_rv32i_bpu_qor_counters.sv
)

TOP="tb_rv32i_bpu_qor_counters"
...
echo
echo "[1/3] Verilator BPU QoR counter lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/bpu_qor_counters_verilator.log

echo
echo "[2/3] Icarus BPU QoR counter compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/bpu_qor_counters_iverilog.log

echo
echo "[3/3] BPU QoR counter self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/bpu_qor_counters.log

echo
echo "BPU QoR counter regression: PASS"
