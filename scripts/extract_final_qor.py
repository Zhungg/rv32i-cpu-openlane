...
def main():
    ...
    if not metrics_file.is_file():
        # Fall back to the last step's state_out.json if final/metrics.json is not present
        step_dirs = sorted(
            [d for d in run.iterdir() if d.is_dir() and d.name[0].isdigit()],
            key=lambda p: int(p.name.split("-")[0]) if p.name.split("-")[0].isdigit() else -1,
            reverse=True,
        )
        found = False
        for s in step_dirs:
            candidate = s / "state_out.json"
        ...
        if not found:
            raise SystemExit(
                f"ERROR: OpenLane metrics not found in {run}"
            )
    ...
    loaded_json = json.loads(metrics_file.read_text(encoding="utf-8"))
    metrics = loaded_json.get("metrics", loaded_json)
    resolved = json.loads(resolved_file.read_text(encoding="utf-8"))

    if not isinstance(metrics, dict):
        raise SystemExit("ERROR: metrics is not a JSON object")
    ...
    utilization_raw, utilization_src = first_numeric(metrics, [
    ...
    if utilization is not None and 0.0 <= utilization <= 1.0:
        utilization *= 100.0
