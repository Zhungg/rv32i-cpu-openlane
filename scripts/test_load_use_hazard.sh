...
SOURCES=(
    ...
    tb/integration/tb_rv32i_load_use_hazard.sv
)

TOP="tb_rv32i_load_use_hazard"
...
echo
echo "[1/3] Verilator load-use hazard lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/load_use_hazard_verilator.log

echo
echo "[2/3] Icarus load-use hazard compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/load_use_hazard_iverilog.log

echo
echo "[3/3] Load-use hazard self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/load_use_hazard.log

echo
echo "Load-use hazard regression: PASS"
