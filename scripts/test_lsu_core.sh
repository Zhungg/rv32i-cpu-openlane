...
SOURCES=(
    ...
    tb/integration/tb_rv32i_lsu_core.sv
)

TOP="tb_rv32i_lsu_core"
...
echo
echo "[1/3] Verilator LSU-core lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/lsu_core_verilator.log

echo
echo "[2/3] Icarus LSU-core compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/lsu_core_iverilog.log

echo
echo "[3/3] LSU-core self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/lsu_core.log

echo
echo "LSU-core integration regression: PASS"
