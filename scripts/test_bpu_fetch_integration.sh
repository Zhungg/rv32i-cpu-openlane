...
SOURCES=(
    ...
    tb/integration/tb_rv32i_bpu_fetch_integration.sv
)

TOP="tb_rv32i_bpu_fetch_integration"
...
echo
echo "[1/3] Verilator BPU fetch integration lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/bpu_fetch_integration_verilator.log

echo
echo "[2/3] Icarus BPU fetch integration compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/bpu_fetch_integration_iverilog.log

echo
echo "[3/3] BPU fetch integration self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/bpu_fetch_integration.log

echo
echo "BPU fetch integration regression: PASS"
