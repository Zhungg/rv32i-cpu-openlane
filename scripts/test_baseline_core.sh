...
SOURCES=(
    ...
    tb/integration/tb_rv32i_baseline_core.sv
)

TOP="tb_rv32i_baseline_core"
...
echo
echo "[1/3] Verilator baseline-core lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/baseline_core_verilator.log

echo
echo "[2/3] Icarus baseline-core compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/baseline_core_iverilog.log

echo
echo "[3/3] Baseline-core self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/baseline_core.log

echo
echo "Baseline-core RTL regression: PASS"
