...
SOURCES=(
    ...
    tb/integration/tb_rv32i_trap_csr.sv
)

TOP="tb_rv32i_trap_csr"
...
echo
echo "[1/3] Verilator CSR trap-state lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/trap_csr_verilator.log

echo
echo "[2/3] Icarus CSR trap-state compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/trap_csr_iverilog.log

echo
echo "[3/3] CSR trap-state self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/trap_csr.log

echo
echo "CSR trap-state regression: PASS"
