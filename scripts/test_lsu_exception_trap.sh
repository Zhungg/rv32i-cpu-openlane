...
SOURCES=(
    ...
    tb/integration/tb_rv32i_lsu_exception_trap.sv
)

TOP="tb_rv32i_lsu_exception_trap"
...
echo
echo "[1/3] Verilator LSU exception-trap lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/lsu_exception_trap_verilator.log

echo
echo "[2/3] Icarus LSU exception-trap compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/lsu_exception_trap_iverilog.log

echo
echo "[3/3] LSU exception-trap self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/lsu_exception_trap.log

echo
echo "LSU exception-trap regression: PASS"
