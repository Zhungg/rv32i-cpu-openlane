...
SOURCES=(
    ...
    tb/integration/tb_rv32i_forwarding_core.sv
)

TOP="tb_rv32i_forwarding_core"
...
echo
echo "[1/3] Verilator forwarding-core lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/forwarding_core_verilator.log

echo
echo "[2/3] Icarus forwarding-core compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/forwarding_core_iverilog.log

echo
echo "[3/3] Forwarding-core self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/forwarding_core.log

echo
echo "Forwarding-core regression: PASS"
