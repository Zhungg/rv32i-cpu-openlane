...
SOURCES=(
    ...
    tb/integration/tb_rv32i_basic_trap.sv
)

TOP="tb_rv32i_basic_trap"
...
echo
echo "[1/3] Verilator basic-trap lint"
...
verilator \
    ...
    --top-module "$TOP" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/basic_trap_verilator.log

echo
echo "[2/3] Icarus basic-trap compilation"
...
iverilog \
    ...
    -o "$OUTPUT" \
    "${SOURCES[@]}" \
    2>&1 | tee reports/lint/basic_trap_iverilog.log

echo
echo "[3/3] Basic-trap self-checking simulation"

vvp "$OUTPUT" \
    2>&1 | tee reports/rtl_sim/basic_trap.log

echo
echo "Basic-trap regression: PASS"
