...
module tb_rv32i_basic_trap;
    ...
    function automatic insn_t program_word(input addr_t address);
        begin
            case (address[9:2])
                ...
                // This must not commit after trap redirect.
                8'd2:
                    program_word =
                        make_i(
                            12'd99,
    ...
    // ------------------------------------------------------------------
    // No data-memory traffic expected in this test.
    // ------------------------------------------------------------------
    ...
    always @(posedge clk) begin
        if (rst_n) begin
            if (baseline_stall) begin
                fail("Unexpected baseline stall during trap test");
            end

            if (dmem_req_valid) begin
                fail("Trap test unexpectedly issued DMEM request");
            end
            ...
            if (commit_valid && commit_trap) begin
                ...
                if (
                    dut.u_datapath.mem_wb_payload.exception.cause !=
                    EXC_ILLEGAL_INSTRUCTION
                ) begin
                    fail("Trap cause mismatch");
                end

                if (
                    dut.u_datapath.mem_wb_payload.exception.tval !=
                    32'h0000_0000
                ) begin
                    fail("Trap tval mismatch");
                end
            end
    ...
    initial begin
        ...
        $display("Wrong-path x2 commits:         0");

        if (error_count != 0) begin
            $fatal(
                1,
                "RV32I basic trap test: FAIL (%0d errors)",
                error_count
            );
        end

        $display("RV32I basic trap test: PASS");
        $finish;
    end
