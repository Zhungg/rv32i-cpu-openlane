...
module tb_rv32i_bpu_qor_counters;
    ...
    initial begin
        ...
        $display("------------------------------------------");
        $display("BPU QoR counters:");
        $display("  branch_count          = %0d", dut.u_datapath.bpu_branch_count_q);
        $display("  mispredict_count      = %0d", dut.u_datapath.bpu_mispredict_count_q);
        $display("  correct_count         = %0d", dut.u_datapath.bpu_correct_count_q);
        $display("  predicted_taken_count = %0d", dut.u_datapath.bpu_predicted_taken_count_q);
        $display("  actual_taken_count    = %0d", dut.u_datapath.bpu_actual_taken_count_q);

        if (dut.u_datapath.bpu_branch_count_q < 8) begin
            fail("BPU branch counter too small");
        end

        if (dut.u_datapath.bpu_mispredict_count_q == 0) begin
            fail("BPU mispredict counter did not increment");
        end

        if (dut.u_datapath.bpu_correct_count_q == 0) begin
            fail("BPU correct counter did not increment");
        end

        if (dut.u_datapath.bpu_predicted_taken_count_q == 0) begin
            fail("BPU predicted-taken counter did not increment");
        end

        if (dut.u_datapath.bpu_actual_taken_count_q < 8) begin
            fail("BPU actual-taken counter too small");
        end

        if (
            dut.u_datapath.bpu_branch_count_q !=
            (
                dut.u_datapath.bpu_mispredict_count_q +
                dut.u_datapath.bpu_correct_count_q
            )
        ) begin
            fail("BPU branch counter does not equal mispredict + correct");
        end

        if (error_count != 0) begin
            $fatal(
                1,
                "RV32I BPU QoR counter test: FAIL (%0d errors)",
                error_count
            );
        end

        $display("RV32I BPU QoR counter test: PASS");
        $finish;
    end
    ...
    initial begin
        ...
        $fatal(
            1,
            "RV32I BPU QoR counter test timeout"
        );
    end
