...
module tb_rv32i_forwarding_core;
    ...
    int unsigned    forward_event_count;
    logic           test_done;
    ...
    function automatic insn_t program_word(input addr_t address);
        begin
            case (address[8:2])
                ...
                // x2 = x1 + 7 = 12
                // Needs EX/MEM -> EX forwarding for rs1.
                7'd1:
                    program_word =
                        make_i(
                            12'd7,
                ...
                // x3 = x2 + x1 = 17
                // Needs EX/MEM for x2 and MEM/WB for x1.
                7'd2:
                    program_word =
                        make_r(
                            FUNCT7_BASE,
                ...
                // sw x6, 0(x10)
                // Needs forwarding of store data x6 and possibly base x10.
                7'd7:
                    program_word =
                        make_s(
                            12'h000,
                ...
                // x8 = x7 + x5 = 0x55 + 12 = 0x61
                // This still relies on Step 6D load-use protection plus
                // normal forwarding/WB-to-ID visibility.
                7'd15:
                    program_word =
                        make_r(
                            FUNCT7_BASE,
    ...
    function automatic xlen_t expected_data(input int unsigned index);
        begin
            case (index)
                ...
                1: expected_data = 32'h0000_000c;
                2: expected_data = 32'h0000_0011;
                3: expected_data = 32'h0000_001d;
                4: expected_data = 32'h0000_000c;
                5: expected_data = 32'h0000_0040;
                6: expected_data = 32'h0000_0055;
                7: expected_data = 32'h0000_0055;
                8: expected_data = 32'h0000_0061;
    ...
    always @(posedge clk) begin
        if (
            rst_n &&
            (
                dut.u_datapath.ex_rs1_forwarded ||
                dut.u_datapath.ex_rs2_forwarded
            )
        ) begin
            forward_event_count++;

            $display(
                "FORWARD[%0d]: rs1=%0b rs2=%0b",
                forward_event_count,
                dut.u_datapath.ex_rs1_forwarded,
                dut.u_datapath.ex_rs2_forwarded
            );
    ...
    initial begin
        ...
        if (forward_event_count == 0) begin
            fail("No forwarding event observed");
        end

        if (data_memory[32'h40 >> 2] !== 32'h0000_0055) begin
        ...
        $display("Forwarding events observed:   %0d", forward_event_count);
        $display("Final memory word @0x40:      %08h", data_memory[32'h40 >> 2]);

        if (error_count != 0) begin
            $fatal(
                1,
                "RV32I forwarding core test: FAIL (%0d errors)",
                error_count
            );
        end

        $display("RV32I forwarding core test: PASS");
        $finish;
    end
    ...
    initial begin
        ...
        $fatal(
            1,
            "RV32I forwarding core test timeout"
        );
    end
